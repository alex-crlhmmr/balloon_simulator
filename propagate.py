...
from weather import get_forecast, gfs_cache
...
class Balloon:
    def __init__(self, radius: float, envelope_mass: float, gas: Union[str, Gas], gas_mass: float = None):
        self.radius = radius
    ...
    def get_reynolds(self, rho: float, T: float, v_rel_norm: float) -> float:
        mu = sutherland_viscosity(T)
        return rho * v_rel_norm * (2 * self.radius) / mu
    
    def get_drag_coeff(self, rho: float, T: float, v_rel_norm: float) -> float:
    ...
    def drag_force(self, rho: float, T: float, v_rel: np.ndarray) -> np.ndarray:
    ...
    def buoyant_force(self, rho: float, T: float, P: float, lat: float, lon: float) -> np.ndarray:
        V = self.get_volume(T, P)
        up = np.array([np.cos(lat) * np.cos(lon), np.cos(lat) * np.sin(lon), np.sin(lat)])
        return up * (rho * V * g)
    
    def gravity_force(self, rho: float, T: float, P: float, lat: float, lon: float) -> np.ndarray:
        total_mass = self.get_total_mass(rho, T, P)
        up = np.array([np.cos(lat) * np.cos(lon), np.cos(lat) * np.sin(lon), np.sin(lat)])
        return -total_mass * g * up

class Payload:
    def __init__(self, radius: float, length: float, mass: float):
        self.radius = radius
        self.length = length
        self.mass = mass
    ...
    def gravity_force(self, lat: float, lon: float) -> np.ndarray:
...
class ForecastCache:
    ...
    def get(self, lat: float, lon: float, alt: float, when: datetime):
        key = (round(lat, 3), round(lon, 3), round(alt, 0), when.replace(microsecond=0, second=0))
    ...
    def set(self, lat: float, lon: float, alt: float, when: datetime, forecast: Dict[str, Any]):
        if len(self.cache) >= self.max_size:
            self.cache.pop(next(iter(self.cache)))
        key = (round(lat, 3), round(lon, 3), round(alt, 0), when.replace(microsecond=0, second=0))
        self.cache[key] = forecast
        # Ensure GFSDataCache has cached columns
        cache_key = (round(lat, 4), round(lon, 4), gfs_cache.cached_time_window)
        if cache_key not in gfs_cache.bilinear_cache:
            gfs_cache.get_column(lat, lon, when)
...
def main(initial_lat=37.428230, initial_lon=-122.168861, initial_height=50.0, num_simulations=1, duration_hours=0.1, num_cpus=1):
    ...
    for i, lat, lon, h, dur in sim_params:
        print(f"Simulation {i}: lat={lat:.6f}, lon={lon:.6f}, h={h:.2f} m, duration={dur:.2f} hours")
    
    if num_simulations == 1:
        run_simulation(*sim_params[0])
    else:
